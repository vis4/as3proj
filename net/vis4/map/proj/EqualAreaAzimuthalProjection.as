...
package net.vis4.map.proj 
{
	...
	public class EqualAreaAzimuthalProjection extends AzimuthalProjection
	{
		...
		private var sinb1:Number;
		private var cosb1:Number;
		private var xmf:Number;
		private var ymf:Number;
		private var mmf:Number;
		private var qp:Number;
		private var dd:Number;
		private var rq:Number;
		public var apa:Array;
		
		public function EqualAreaAzimuthalProjection() 
		{
			initialize();
		}
		...
		{
			var p:EqualAreaAzimuthalProjection = super.clone() as EqualAreaAzimuthalProjection;
			p.apa = [];
			for each (var n:Number in apa) p.apa.push(n);
			return p;
		}
		...
		override public function project(lam:Number, phi:Number, xy:Point):Point 
		{
			if (spherical) {
				...
				case EQUATOR:
					...
					if (xy.y <= EPS10) throw new ProjectionError();
				...
				case NORTH_POLE:
					coslam = -coslam;
				case SOUTH_POLE:
					if (Math.abs(phi + projectionLatitude) < EPS10) throw new ProjectionError();
				...
				var sinlam:Number, q:Number, sinb:Number = 0, cosb:Number = 0, b:Number = 0;
				...
				if (Math.abs(b) < EPS10) throw new ProjectionError();
				switch (mode) {
				case OBLIQUE:
		...
		{
			if (spherical) {
				var cosz:Number = 0, rh:Number, sinz:Number = 0;
				...
				var cCe:Number, sCe:Number, q:Number, rho:Number, ab:Number = 0;
		...
		override public function get shortName():String { return "Lambert III"; }
	}
