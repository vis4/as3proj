...
package net.vis4.map.proj 
{
	...
	{
		private static const TOL:Number = 1.0e-7;

		private var alpha:Number, lamc:Number, lam1:Number, phi1:Number, lam2:Number, phi2:Number, 
			Gamma:Number, al:Number, bl:Number, el:Number, singam:Number, cosgam:Number, sinrot:Number, 
			cosrot:Number, u_0:Number;
		private var ellips:Boolean, rot:Boolean;
	
		public function ObliqueMercatorProjection(ellipsoid:Ellipsoid = null, lon_0:Number = undefined, 
			lat_0:Number = undefined, alpha:Number = undefined, k:Number = undefined, x_0:Number = undefined, 
			y_0:Number = undefined) 
		{
			if (ellipsoid == null) {
				...
				alpha = MapMath.toRadians(-45);//FIXME
				initialize();
			} else {
		...
		{
			super.initialize();
			var con:Number, com:Number, cosphi0:Number, d:Number, f:Number, h:Number, l:Number, sinphi0:Number, p:Number, j:Number;
			var azi:int = 1;//FIXME-param
			...
			if (azi != 0) {//alpha specified
				if (Math.abs(alpha) <= TOL ||
					...
					throw new ProjectionError("Obl 1");
			} else {
				if (Math.abs(phi1 - phi2) <= TOL ||
					(con = Math.abs(phi1)) <= TOL ||
					Math.abs(con - MapMath.HALFPI) <= TOL ||
					Math.abs(Math.abs(projectionLatitude) - MapMath.HALFPI) <= TOL ||
					Math.abs(Math.abs(phi2) - MapMath.HALFPI) <= TOL) throw new ProjectionError("Obl 2");
			}
	...
	//		u_0 = MapMath.param(params, "bno_uoff").i ? 0. :
		...
		{
			var con:Number, q:Number, s:Number, ul:Number, us:Number, vl:Number, vs:Number;
		...
		{
			var q:Number, s:Number, ul:Number, us:Number, vl:Number, vs:Number;
		...
		override public function get shortName():String { return "Obl. Mercator"; }
